...
class SnakeState extends State<Snake> {
  ...
  double cellSize;
  GameState? state;
  GravityEvent? acceleration;
  late StreamSubscription<GravityEvent> _streamSubscription;
  late Timer _timer;
  ...
  void initState() {
    super.initState();
    _streamSubscription =
        accelerometerEventStream().listen((GravityEvent event) {
      setState(() {
        acceleration = event;
      });
    });
    ...
    final newDirection = acceleration == null
        ...
        : acceleration!.vector.x.abs() < 1.0 &&
            ...
            : (acceleration!.vector.x.abs() < acceleration!.vector.y.abs())
                ...
                : math.Point<int>(-acceleration!.vector.x.sign.toInt(), 0);
